...
export const Pagination = ({ pagesCount, setPageNumber, pageNumber }) => {
  const numberOfPages = range(1, pagesCount);
  const visibleCount = 5;

  const getSliceStart = () => {
    if (range(0, Math.ceil(visibleCount / 2)).includes(pageNumber)) {
  ...
  const getSliceEnd = () => {
    ...
    } else if (range(0, Math.ceil(visibleCount / 2)).includes(pageNumber)) {
      return visibleCount;
    } else {
